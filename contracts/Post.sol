...
contract Post is Ownable {
    ...
    function calculateSellerPledge(
        ...
        (sellerPledge, sellerTxFee) = PostLib.calculateSellerPledge(
            price,
            amount,
            sellerRatio,
            sellerRate
    ...
    function calculateBuyerPledge(
        ...
        (buyerPledge, buyerTxFee) = PostLib.calculateBuyerTxFeeAndPledge(
            price,
            amount,
            buyerRate
        );
    ...
    function addOrder(
     ...
     (uint256 _buyer_pledge, uint256 _buyer_tx_fee) = calculateBuyerPledge(
        ...
        //2、将代币转入到合约地址
        IERC20(_token).transferFrom(_user, address(this), _buyer_pledge);
        orders.push(
            Order({
                name: _name,
                seller: _seller,
    ...
    function place(
        ...
        //3、校验订单是否指定卖家
        require(
            order.seller == address(0) || order.seller == _user,
        ...
        if (_amount == order.amount) {
            ...
            // uint256 _buyer_pledge_new = PostLib.divider(
            //     _amount,
            //     order.amount,
            //     order.buyer_pledge
            // );
            ...
            sellerList[_user].push(_order_id_new);
    ...
    function confirm(uint256 _orderId) external {
        ...
        (
            ...
            uint256 sellerBack,
            uint256 buyerBack
        ) = PostLib.confirmCalculateRefunds(
                order.seller_pledge,
                order.buyer_pledge,
                order.price,
                order.amount,
    ...
    function getContact(
        ...
        if (
            _user == owner() ||
            PostLib.contains(sellerList[_user], _orderId) ||
            PostLib.contains(buyerList[_user], _orderId)
        ) {
